...
. STSのメニューバーから、[Window] -> [Perspective] -> [Open Perspective] -> [Other...]を選択する。
. [DBeaver]を選択し、[Open]を押下する。
. STSのメニューバーに[SQLエディタ]と[データベース]があることを確認する。

.STS Menu Bar
image::ch09/EnvironmentConstruction/Ch09_EnvironmentConstruction_DBeaver_Perspective_Menubar.png[STS Menu Bar]
