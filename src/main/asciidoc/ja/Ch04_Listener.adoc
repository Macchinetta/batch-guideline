...
例外ハンドリングについては、<<Ch06_ExceptionHandling.adoc#Ch06_ExceptionHandling_HowToUse_StepExceptionHandling_ChunkListener,``ChunkListener``インタフェースによる例外ハンドリング>>を参照。
...
[WARNING]
.タスクレットモデルではアノテーションによるリスナー実装が利用できない
====
タスクレットモデルの場合は、<<Ch04_Listener_HowToUse_Impl_WithoutAnnotation>>を参考に、リスナーインタフェースを利用すること。アノテーションによるリスナー実装をタスクレットモデルに設定しても、リスナーを起動することができない。
====

[[Ch04_Listener_HowToUse_Configuration]]
=== リスナーの設定
リスナーは、Bean定義の``<batch:listeners>``.``<batch:listener>``要素によって設定する。
