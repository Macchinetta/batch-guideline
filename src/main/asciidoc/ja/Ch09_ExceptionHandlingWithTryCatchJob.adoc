...
なお、詳細については{batch5_guide}の<<Ch06_ExceptionHandling.adoc#Ch06_ExceptionHandling_HowToUse_StepExceptionHandling_Chunk_ItemProcessor_TryCatchExample,ItemProcessor内でtry～catchする方法>>および
<<Ch06_ExceptionHandling.adoc#Ch06_ExceptionHandling_HowToUse_StepExceptionHandling_Tasklet,タスクレットモデルにおける例外ハンドリング>>を参照。
...
. ステップは``StepExitStatusChangeListener``を実行する。
. ``StepExitStatusChangeListener``は、入力データと出力データの件数が異なる場合に``StepExecution``に独自の終了コードとして``SKIPPED``を設定する。
...
ItemReader、ItemWriterでのスキップは<<Ch06_ExceptionHandling.adoc#Ch06_ExceptionHandling_HowToUse_ContinuationPropriety_Skip,スキップ>>を参照。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup09040501" class="JavaConfigTab" id="tabgroup09040501_1" checked><label for="tabgroup09040501_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup09040501" class="XMLConfigTab" id="tabgroup09040501_2"><label for="tabgroup09040501_2" class="tab">XMLConfig</label>
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup09040502" class="JavaConfigTab" id="tabgroup09040502_1" checked><label for="tabgroup09040502_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup09040502" class="XMLConfigTab" id="tabgroup09040502_2"><label for="tabgroup09040502_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
ここでは、``StepExecutionListener``の実装クラスのBeanIDである``stepExitStatusChangeListener``を指定する。
...
====
``StepExitStatusChangeListener``はステップの実行前後に処理を割り込める``StepListener``に属するため、``<batch:tasklet>``要素内に``<batch:listeners>``.``<batch:listener>``要素によって設定する。 +
``JobExitCodeChangeListener``はジョブの実行前後に処理を割り込める``JobListener``に属するため、``<batch:job>``要素内に``<batch:listeners>``.``<batch:listener>``要素によって設定する。

詳細は<<Ch04_Listener.adoc#Ch04_Listener_HowToUse_Configuration,リスナーの設定>>を参照。
====
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup09040503" class="JavaConfigTab" id="tabgroup09040503_1" checked><label for="tabgroup09040503_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup09040503" class="XMLConfigTab" id="tabgroup09040503_2"><label for="tabgroup09040503_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
``LaunchContextConfig.java``に以下のとおり、独自の終了コードを追加する。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup09040504" class="JavaConfigTab" id="tabgroup09040504_1" checked><label for="tabgroup09040504_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup09040504" class="XMLConfigTab" id="tabgroup09040504_2"><label for="tabgroup09040504_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup09040505" class="JavaConfigTab" id="tabgroup09040505_1" checked><label for="tabgroup09040505_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup09040505" class="XMLConfigTab" id="tabgroup09040505_2"><label for="tabgroup09040505_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
|(1)
|コンポーネントスキャン対象とするベースパッケージの設定を行う。 +
``basePackages属性/base-package属性``に、``StepExecutionListener``および``JobExecutionListener``の実装クラスが格納されているパッケージを追加で指定する。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup09040506" class="JavaConfigTab" id="tabgroup09040506_1" checked><label for="tabgroup09040506_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup09040506" class="XMLConfigTab" id="tabgroup09040506_2"><label for="tabgroup09040506_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
``LaunchContextConfig.java``に以下のとおり、独自の終了コードを追加する。
...
<<Ch09_Impl_ExceptionHandlingWithTryCatchJob_Prerequisite>>のとおりデータベースアクセスするジョブの場合の説明となるため、
ファイルアクセスするジョブの場合の実装は以下の(1)～(7)のみ追加する。
