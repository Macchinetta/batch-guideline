...
``StaxEventItemWriter``ではデフォルト値をそのまま使用する意図である場合でも明示的にエンコーディングを設定することを推奨する。
...
|``StaxEventItemReader``
|なし
|入力するXMLのencoding属性に従う。 +
encoding属性が宣言されていない場合はUTF-8となる。
...
====
{SB} 4.3.0以降では``StaxEventItemReader``にencodingプロパティが追加され、Bean定義でのエンコーディングの指定が可能となった。
これに合わせ、デフォルト値も``FlatFileItemReader``等と同様にJavaVMのデフォルトエンコーディングに従うよう変更されている。

Macchinetta Batch 2.3.0は{SB} 4.3.1を利用しているため、Macchinetta Batch 2.3.0へのバージョンアップを行う場合は、
