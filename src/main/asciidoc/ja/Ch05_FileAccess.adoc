...
[IMPORTANT]
====
フッタレコードを読み飛ばすことは、{SB}では機能をもっていないため、JIRAの  https://jira.spring.io/browse/BATCH-2539[{SB}/BATCH-2539]  にて報告している。 +
よって、今後OSコマンドによる読み飛ばしだけではなく、{SB}でも対応できるようになる可能性がある。
====
...
|(1)
|XML のルートタグとするため、``@XmlRootElement``アノテーションを付与する。 +
タグの名前に``SalesPlanDetail``を設定する。
|===
