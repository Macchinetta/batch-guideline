...
ここには、DTO、TaskletやProcessorの実装、MyBatis3の``Mapper``インタフェースを格納する。 +
...
|(12)
|MyBatis3の``Mapper``インタフェースの対となるMapper XMLファイル。
...
このため、JavaConfigでBeanのインジェクションを行う場合、``@Bean``アノテーションが付与されたメソッドの引数を使用し、コンストラクタやフィールドに対するインジェクションを使用しないこと。
====
...
JavaConfigでは、デフォルトではBeanの特定は型名で行われる。そのため、同じ型をもつBeanが複数定義されていると、Beanが特定できずエラーとなる。エラーを回避するために、一意なBeanIDを明示する``@Qualifier``アノテーションを付与する。

なお、{batch5_shortname}では、次に紹介するケースで``@Qualifier``アノテーションの付与が必須となる。

==== データソース、トランザクションマネージャをインジェクションするとき

{batch5_shortname}では、データソース、トランザクションマネージャは、同じ型をもつ複数のBean定義が存在する。そのため、これらのBeanをインジェクションするときは、``@Qualifier``アノテーションでBeanIDを指定しBeanを特定する必要がある。
...
.2+.^|``org.apache.commons.dbcp2.BasicDataSource``

|``jobDataSource``
|ジョブが利用するデータソース

|``adminTransactionManager``
|{SB}や{batch5_shortname}が利用するトランザクションマネージャ
...
以下はジョブ定義で``jobTransactionManager``を``@Qualifier``アノテーション付きでインジェクションする例となる。
...
[cols="10,90", options="header"]
.jobTransactionManagerのインジェクション
|===
|項番
|説明

|(1)
|``Bean``メソッドの引数``transactionManager``に``@Qualifier``アノテーションを付与する。その際、BeanID（``jobTransactionManager``）を指定する。
|===

==== ジョブのフロー制御や並列処理で複数のステップを定義するとき

フロー制御や並列処理では複数のステップを定義するため、型（``org.springframework.batch.core.Step``）が重複する。ジョブのBean定義にステップをインジェクションする際に、``@Qualifier``アノテーションを付与する。

以下は複数のステップを``@Qualifier``アノテーション付きでインジェクションする例となる。
...
|(1)
|``Bean``メソッドの引数``stepA``に``@Qualifier``アノテーションを付与する。その際、BeanID（``stepA``）を指定する。

|(2)
|``Bean``メソッドの引数``stepB``に``@Qualifier``アノテーションを付与する。その際、BeanID（``stepB``）を指定する。

|(3)
|``Bean``メソッドの引数``stepC``に``@Qualifier``アノテーションを付与する。その際、BeanID（``stepC``）を指定する。
|===
...
<profiles>
    ...
    <!-- omitted -->
    <profile>
        <id>example-db-local</id>
        <properties>
            <jdbc.driver.groupId>com.example.database.jdbc</jdbc.driver.groupId>
            <jdbc.driver.artifactId>jdbc-driver</jdbc.driver.artifactId>
            <!-- omitted -->
        </properties>
    </profile>
</profiles>
...
* https://docs.spring.io/spring-batch/docs/{spring_batch_version}/api/org/springframework/batch/core/configuration/support/DefaultBatchConfiguration.html[クラス DefaultBatchConfiguration]
...
``TerasolunaBatchConfiguration``は、``DefaultBatchConfiguration``のBean定義をカスタマイズするために用意されている。Bean定義のカスタマイズは、``DefaultBatchConfiguration``の``Bean``メソッドをオーバーライドすることで実現する。
