...
icon:warning[] {SB}が提供するコンポーネントのエンコーディングのデフォルト値はItemReaderとItemWriterで異なっている(ItemWriterのデフォルト値は「UTF-8」)。 +
そのため、デフォルト値をそのまま使用する意図である場合でも明示的にエンコーディングを設定することを推奨する。
|
...
icon:warning[] {SB}が提供するコンポーネントのエンコーディングのデフォルト値はItemReaderとItemWriterで異なっている(ItemReaderのデフォルト値は「JavaVMのデフォルトエンコーディング」)。 +
そのため、デフォルト値をそのまま使用する意図である場合でも明示的にエンコーディングを設定することを推奨する。
|
|UTF-8
...
また、JDK11およびそれ以降でJAXBを利用する場合、jaxb-runtimeの追加も必要である。 +
アプリケーションの依存ライブラリやバッチから提供されるライブラリにjaxb-runtimeがない場合は、pom.xmlのJDKのプロファイル定義の中に下記の依存関係を追加する。 +
ここでは、JDK11においてブランクプロジェクトのpom.xmlにjaxb-runtimeの依存性を追加する例を示す。
[source, xml]
----
<profiles>
  ... ommited
...
|(1)
|jaxb-runtimeはアプリケーションの実行時にのみ利用されるため、スコープはruntimeとする。
|===
...
fileaccess.schema-file-path=files/test/input/ch05/fileaccess/customer.xsd
----
[cols="10,90", options="header"]
.設定内容の項目一覧
|===
...
|出力ファイルのエンコーディングを設定する。 +
<<Ch05_FileAccess_HowToExtend_Xml_Encoding>>で述べたように、ItemReader/Writer間のデフォルト値の違いによって意図しないエンコーディングで
出力が行われることを防ぐため、デフォルト値をそのまま使用する意図である場合でも明示的にエンコーディングを設定することを推奨する。
|
|UTF-8
...
[WARNING]
.JDK8の環境では現在``standalone``プロパティの指定が不可能となっている
====
JDK8の場合、``standalone``を設定してもXMLファイルに出力されない。 +
これはJDK8の不具合として報告され現在解消しているが、JDK9の対応となっているためである。 +
https://bugs.openjdk.org/browse/JDK-8139584[JDK Bug System/JDK-8139584] +
{SB}によるこの不具合報告の状況としては以下を参照されたい。 +
https://github.com/spring-projects/spring-batch/issues/758[{SB}/BATCH-2856] +
====
