...
https://docs.spring.io/spring-boot/docs/2.2.12.RELEASE/reference/htmlsingle/[2.2.12.RELEASE]である。
====
...
基盤フレームワーク::
  ...
  ** https://docs.spring.io/spring-framework/docs/{spring_fw_version}/spring-framework-reference/core.html[Spring Framework 5.2]

バッチフレームワーク::
  バッチフレームワークとして、{SB}を利用する。同期実行には{SB}が提供するCommandLineJobRunnerを利用する。

  ** https://docs.spring.io/spring-batch/docs/{spring_batch_version}/reference/html/index.html[{SB} 4.2]
...
非同期実行::
  ...
  *** https://docs.spring.io/spring-framework/docs/{spring_fw_version}/spring-framework-reference/web.html#mvc[Spring MVC 5.2]
...
バリデーション::
  単項目チェック:::
    単項目チェックにはBean Validationを利用し、実装はHibernate Validatorを使用する。
      ...
      *** https://docs.jboss.org/hibernate/validator/6.0/reference/en-US/html_single/[Hibernate Validator 6.0]

  相関チェック:::
    相関チェックにはBean Validation、もしくはSpring Validationを利用する。

      *** https://docs.spring.io/spring-framework/docs/{spring_fw_version}/spring-framework-reference/core.html#validator[Spring Validation 5.2]
