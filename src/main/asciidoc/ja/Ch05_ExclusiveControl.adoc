...
ただし、ItemStreamReader/ItemStreamWriterには、ファイルロックを取得する機能がない。
そのため、排他対象のファイルと一対一に、排他対象ファイルのロックを担当するファイル(以降、ロック用ファイル)を用意し、ロック用ファイルからファイルロックを取得する。
...
[NOTE]
.ItemWriterのtransactionalプロパティについて
====
サンプルコードにおける、ItemStreamWriterの``transactional``プロパティは``false``を設定している。
``transactional``プロパティが``true``の場合、TransactionManagerと同期してリソースを出力することに注意すること。
詳細は、<<Ch05_Transaction.adoc#Ch05_Transaction,トランザクション制御>>を参照。
====
