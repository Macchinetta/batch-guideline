...
<mapper namespace="jp.co.ntt.fw.macchinetta.batch.functionaltest.ch05.exclusivecontrol.repository.ExclusiveControlRepository">
    ...
    <!-- (7) -->
    <update id="branchExclusiveUpdate"
            parameterType="jp.co.ntt.fw.macchinetta.batch.functionaltest.ch05.exclusivecontrol.model.ExclusiveBranch">
        <![CDATA[
	        UPDATE
	            branch_mst
	        SET
...
|(9)
|返り値の型は、このクラスで実装しているItemProcessorインタフェースの型引数で指定した 出力オブジェクトの型である``ExclusiveBranch``とする。
