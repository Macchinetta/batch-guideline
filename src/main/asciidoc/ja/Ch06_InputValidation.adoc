...
Hibernate ValidatorはBean Validationで定義されたアノテーションに加え、独自の検証用アノテーションを提供
...
* ``ItemProcessor``に、``org.springframework.batch.item.validator.Validator``の実装である
  ``org.springframework.batch.item.validator.SpringValidator``をインジェクションし``validate``メソッドを実行する。
** ``SpringValidator``は内部に``org.springframework.validation.Validator``を保持し、``validate``メソッドを実行する。 +
...
. 例外をtry/catchで捕捉せず、``ItemProcessListener``を実装し``onProcessError``メソッドにて行う。
...
``onProcessError``メソッドにて発生した例外の種類を判定して例外処理を行う必要がある。
