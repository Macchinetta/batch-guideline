...
.``FlatFileItemReaderBuilder``クラスでの``name``メソッドの使用
...
**``name``メソッドで``ItemStreamSupport``のサブクラス名を指定する理由**
...
- https://docs.spring.io/spring-batch/docs/{spring_batch_version}/api/org/springframework/batch/item/file/builder/FlatFileItemReaderBuilder.html[クラス FlatFileItemReaderBuilder<T>]
- https://docs.spring.io/spring-batch/docs/{spring_batch_version}/api/org/springframework/batch/item/file/builder/FlatFileItemWriterBuilder.html[クラス FlatFileItemWriterBuilder<T>]
- https://docs.spring.io/spring-batch/docs/{spring_batch_version}/api/org/springframework/batch/item/file/builder/MultiResourceItemReaderBuilder.html[クラス MultiResourceItemReaderBuilder<T>]
- https://docs.spring.io/spring-batch/docs/{spring_batch_version}/api/org/springframework/batch/item/file/builder/MultiResourceItemWriterBuilder.html[クラス MultiResourceItemWriterBuilder<T>]
- https://docs.spring.io/spring-batch/docs/{spring_batch_version}/api/org/springframework/batch/item/xml/builder/StaxEventItemReaderBuilder.html[クラス StaxEventItemReaderBuilder<T>]
- https://docs.spring.io/spring-batch/docs/{spring_batch_version}/api/org/springframework/batch/item/xml/builder/StaxEventItemWriterBuilder.html[クラス StaxEventItemWriterBuilder<T>]
...
|delimiter
|区切り文字を設定する。 +
TSVファイルの読み込みを行う場合には、<<Ch05_FileAccess_TSV_Input>>を参照。
|
|カンマ
...
ファイル出力前に出力ファイルが存在していた場合の動作を設定する。 +
trueの場合、ファイル出力前に既にファイルが存在すれば削除する。 +
falseの場合、ファイル出力前に既にファイルが存在すれば例外をスローする。
|
|true
...
trueの場合、ファイル出力後に出力件数が0件であれば出力対象ファイルを削除する。 +
icon:warning[] 他の設定との組み合わせによって意図しない動作をする場合がある。詳細は<<Ch05_FileAccess_HowToUse_VariableLength_Output_Property,FlatFileItemWriterのshouldDeleteIfExistsとshouldDeleteIfEmptyの動作の違い>>を参照。
|
|false
...
[[Ch05_FileAccess_EnclosableDelimitedLineAggregator]]
.EnclosableDelimitedLineAggregatorについて
...
この拡張クラスでは、{SB}により提供されている``org.springframework.batch.item.file.transform.DelimitedLineAggregator``に、フィールドを囲み文字で囲む処理に対応することができ、RFC-4180 で求められている要件を満たすための拡張が行われている。
本機能の実装の経緯については、 https://github.com/spring-projects/spring-batch/issues/1139[{SB}/BATCH-2463] を参照。 +
...
``EnclosableDelimitedLineAggregator``の使用方法は<<Ch05_FileAccess_EnclosableDelimitedLineAggregator>>を参照。
|icon:check[]
|なし
...
|delimiter
|区切り文字を設定する。 +
TSVファイルの出力を行う場合には、<<Ch05_FileAccess_TSV_Output>>を参照。
|
|カンマ
...
[[Ch05_FileAccess_HowToUse_VariableLength_Output_Property]]
==== FlatFileItemWriterのshouldDeleteIfExistsとshouldDeleteIfEmptyの動作の違い

``FlatFileItemWriter``のプロパティである、``shouldDeleteIfExists``と``shouldDeleteIfEmpty``は実行タイミングや判定対象が異なる。

image::ch05/FileAccess/Ch05_FlatFileItemWriter_Property.png["FlatFileItemWriter Property",align="center"]

[cols="10,30,15,15,30", options="header"]
.shouldDeleteIfExistsとshouldDeleteIfEmptyの違い
|===
|
|機能
|判定タイミング
...
|===
====
*1. 特定条件下では出力件数が0件ではない場合でもファイルが削除される場合がある、詳細は後述の「<<Ch05_FileAccess_HowToUse_VariableLength_Output_Property_shouldDeleteIfEmpty_true,``shouldDeleteIfEmpty``がtrueの場合の注意すべき動作>>」を参照。
====

``shouldDeleteIfExists``はファイル出力前の出力ファイルを対象とし、``shouldDeleteIfEmpty``はファイル出力後の出力件数を対象をしている。

[[Ch05_FileAccess_HowToUse_VariableLength_Output_Property_shouldDeleteIfEmpty_true]]
``shouldDeleteIfEmpty``がtrueの場合の注意すべき動作について説明する。 +
以下の条件を満たした場合、出力件数が0件でなくとも出力ファイルが削除されるため注意が必要である。

* ``shouldDeleteIfExists``にfalseを設定
* ``shouldDeleteIfEmpty``にtrueを設定
* Jobによるファイル出力前に既に出力ファイルが存在する

条件を満たした場合、以下の様に処理が進む。

. ``shouldDeleteIfExists``はジョブ開始時に既存ファイルがあるかチェックされる。
. この時点ではまだ``ItemWriter``によるファイル出力前だが、同名のファイルが既に存在したときはファイル出力を行われずに例外がスローされる。
. しかし例外スロー前に後処理としてファイルクローズが行われ、ジョブ終了時の``shouldDeleteIfEmpty``が評価される。
. ここで``shouldDeleteIfEmpty``がtrueの場合、前処理で例外がスローされているためジョブの出力件数が0件となりファイルが削除される。 +
これは既存ファイルのレコードがたとえ10000行あっても出力件数が0件として評価されファイルが削除される。

このように、``shouldDeleteIfExists``と``souldDeleteIfEmpty``の設定値の組み合わせによっては、出力ファイルが意図せず削除される恐れがあり注意が必要である。 +
意図せぬ出力ファイルの削除を避けるため、以下を検討するとよい。

* 不要な出力ファイルは、別の方法で削除する
** ジョブスケジューラ等で、不要な出力ファイルを定期的にクリーンアップする
* ``shouldDeleteIfExists``、``shouldDeleteIfEmpty``による出力ファイル削除を禁止する
** ``shouldDeleteIfExists``、``shouldDeleteIfEmpty``はともにfalseとする

以上のことから、``shouldDeleteIfExists``と``shouldDeleteIfEmpty``は実行タイミングや判定対象が異なることと、特定条件下で出力ファイルが意図せず削除される恐れがあるため注意が必要である。
...
[[Ch05_FileAccess_HowToUse_HeaderFooter_batchListeners]]
===== リスナー設定について
...
下記のような西暦フォーマットのDate型やカンマを含むBigDecimal型の変換を行うファイルを読み込む場合の実装例を示す。
...
"000003","2018年3月3日","000000003","3,000,000,000"
----
...
西暦フォーマットの日付をDate型へ変換するため、``SimpleDateFormat``でフォーマットを指定している。
...
p:schema="file:/usr/local/terasoluna/functionaltest/files/test/input/ch05/fileaccess/customer.xsd"
----
...
この例では、スキーマファイルの場所を``batch-application.properties``に定義し、``@Value``を使用してプロパティとしてインジェクションする方式とする。 +
...
後述で示される<<output_example,出力ファイル例>>を書き出すための設定例を示す。

.依存ライブラリの追加
依存ライブラリを追加する際の設定方法は<<Ch05_FileAccess_HowToExtend_Xml_Input>>で説明されている<<add_library,依存ライブラリの追加>>を参照する。
...
後述の<<output_example,出力ファイル例>>で示されているように、XMLのフォーマット処理を行うためには、以下のように ``Jaxb2Marshaller`` に対して ``marshallerProperties`` を設定する必要がある。
...
後述の<<output_example,出力ファイル例>>を書き出すための設定は以下のとおり。
...
ここでは、フォーマット済みの出力を行うために``javax.xml.bind.Marshaller.JAXB_FORMATTED_OUTPUT``をtrueに設定している。
|
|なし
|===
...
下記は<<Ch05_FileAccess_HowToExtend_Xml_Output, XMLファイルの出力>>でStaxEventItemWriterを使用したXMLファイルの出力例である。
[[output_example]]
[source, xml]
.出力ファイル例
----
