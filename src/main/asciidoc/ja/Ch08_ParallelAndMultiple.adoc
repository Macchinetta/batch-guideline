...
|(2)
|``<batch:split>``要素内に並列実行するステップを``<batch:flow>``要素を使用した形式で定義をする。 +
...
``step``属性には、(12)で定義するWorkerステップのBeanIDを設定する。
...
|(3)
|``resouce``属性に、``MultiResourcePartitioner``で分割されたファイルを入力ファイルに指定する。 +
...
|データの分割条件を生成する処理を定義する。 +
``partitioner``属性には、``Partitioner``インタフェース実装を設定する。 +
``step``属性には、(15)で定義するWorkerステップのBeanIDを設定する。
