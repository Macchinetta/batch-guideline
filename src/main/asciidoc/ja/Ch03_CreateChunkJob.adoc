...
@ComponentScan("jp.co.ntt.fw.macchinetta.batch.functionaltest.app.common") // (2)
@MapperScan(value = "jp.co.ntt.fw.macchinetta.batch.functionaltest.app.repository.mst", sqlSessionFactoryRef = "jobSqlSessionFactory") // (3)
...
``ItemProcessor``は、以下のインタフェースが示すとおり、入力リソースから取得したデータ**1件**をもとに、
出力リソースに向けたデータ**1件**を作成する役目を担う。
つまり、``ItemProcessor``はデータ**1件**に対するビジネスロジックを実装する箇所、と言える。
...
[[Ch03_CreateChunkJob_HowToExtend]]
== How to extend

[[Ch03_CreateChunkJob_ItemProcessor]]
ItemProcessorの省略::
入力データを加工せず、そのまま出力する場合は、``ItemProcessor``を省略するか、``PassThroughItemProcessor``を利用する。
以下には``ItemProcessor``を省略する設定例となる。
// ============ Java/XML Config Tab start. ============
[source,java,indent=0,role="primary"]
.JavaConfig
====
