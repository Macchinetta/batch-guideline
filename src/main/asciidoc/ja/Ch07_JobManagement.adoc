...
[[Ch07_JobManagement_Overview_JobExecutionManagement_PerformanceImprovement_ByDao]]
[TIP]
.DAOの実装によるJobRepositoryの性能改善
====
{SB} 4.2.xから加わった変更により、ジョブの起動時に{SB}が発行する``JobRepository``へのlink:https://github.com/spring-projects/spring-batch/issues/3634[SQLによって、性能劣化]が発生する事がある。

本件は{SB}側で対処されており、{SB} 4.3.0で取り込まれる予定である。一方で、Macchinetta Batch {revnumber-index}が利用する{SB} {spring_batch_version}には取り込まれていないため、本バージョンを利用する場合は、本件に起因した性能劣化が発生する可能性がある。

本件の影響は、ショートジョブの大量実行などによって``JobRepository``に大量のレコードがある状態で顕著に表れる。そのため、類似するバッチジョブを実装している場合、DBの実行計画を取得するなどして本件の影響を確認してほしい。

本件の対策としては、以下の対応が考えられる。

. {SB}で対処されたlink:https://github.com/spring-projects/spring-batch/pull/3635/files[変更]を実装する
. <<Ch04_AsyncJobWithDB.adoc#Ch04_AsyncJobWithDB_HowToUse_CumulativeData,``JobRepository``に蓄積されるレコードを一定間隔で退避>>する

1の方法を選択する場合、link:https://github.com/spring-projects/spring-batch/pull/3635#issuecomment-611571008[こちら]のコメントを参考に、{SB}のクラスをオーバーライドしてほしい。
====
...
<batch:step id="exitstatusjob.step">
    ...
    <batch:listeners>
        <batch:listener ref="stepExitStatusChangeListener"/>
    </batch:listeners>
</batch:step>
----
