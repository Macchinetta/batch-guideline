...
|(1)
|``<batch:tasklet>``要素の``transaction-manager``属性に定義済みの``jobTransactionManager``を設定する。 +
...
|(1)
|``<batch:tasklet>``要素の``transaction-manager``属性に定義済みの``jobResourcelessTransactionManager``を設定する。
...
|(9)
|ItemProcessorの``process``メソッド内で、マスタデータからデータ取得を行う。
