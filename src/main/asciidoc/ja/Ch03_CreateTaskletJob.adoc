...
|(1)
|``org.springframework.batch.core.step.tasklet.Tasklet``インターフェースを``implements``して実装する。

|(2)
|``Tasklet``インターフェースが定義する``execute``メソッドを実装する。
...
|(3)
|``ItemReader``のサブインターフェースである、``ItemStreamReader``として型を定義する。 +
