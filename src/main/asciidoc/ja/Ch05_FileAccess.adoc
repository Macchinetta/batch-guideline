...
.データ部のフォーマットに関する前提
====
<<Ch05_FileAccess_HowToUse>>では、データ部のレコードが同一のフォーマットである事を前提として説明する。 +
...
* マルチフォーマットや上記の構造にフッタ部を含む構造をもつフラットファイルについては、<<Ch05_FileAccess_HowToExtend>>を参照。
====
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050301" class="JavaConfigTab" id="tabgroup050301_1" checked><label for="tabgroup050301_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050301" class="XMLConfigTab" id="tabgroup050301_2"><label for="tabgroup050301_2" class="tab">XMLConfig</label>
...
====
``FlatFileItemReaderBuilder``は、``FlatFileItemReader``を生成するBuilderクラスである。

本章で登場する以下のBuilderクラスでは、デフォルトで``name``メソッドの使用が必須となっている。
...
``name``メソッドでは、``ExecutionContext``に実行状態を保存する際のキー名を引数に設定する。キー名は、特段の理由がないかぎり、``ItemStreamSupport``のサブクラス名を指定する。``ItemStreamSupport``のサブクラスとは、Builderクラスで生成するクラスのことである。

**nameメソッドでItemStreamSupportのサブクラス名を指定する理由**

``ExecutionContext``やログに出力される名前を、その出力元のクラス名と一致させておくことで、トレーサビリティを担保する。これは障害解析時等に、原因箇所特定の手がかりとするためである。

**FlatFileItemReaderBuilderでの設定例**

``ItemStreamSupport``のサブクラス名は``org.springframework.batch.item.file.FlatFileItemReader``の短縮名、すなわち``"FlatFileItemReader"``となる。下記のように設定する。
...
なお、Builderクラスを使用せず直接``FlatFileItemReader``をnewしたときも、デフォルトでコンストラクタ内で``ItemStreamSupport``のサブクラス名（``"FlatFileItemReader"``）が設定される（以下）。
...
``name``メソッドについては以下の公式リファレンスを参照のこと。

- https://spring.pleiades.io/spring-batch/docs/current/api/org/springframework/batch/item/file/builder/FlatFileItemReaderBuilder.html#name(java.lang.String)[クラス FlatFileItemReaderBuilder<T>]
- https://spring.pleiades.io/spring-batch/docs/current/api/org/springframework/batch/item/file/builder/FlatFileItemWriterBuilder.html#name(java.lang.String)[クラス FlatFileItemWriterBuilder<T>]
- https://spring.pleiades.io/spring-batch/docs/current/api/org/springframework/batch/item/file/builder/MultiResourceItemReaderBuilder.html#name(java.lang.String)[クラス MultiResourceItemReaderBuilder<T>]
- https://spring.pleiades.io/spring-batch/docs/current/api/org/springframework/batch/item/file/builder/MultiResourceItemWriterBuilder.html#name(java.lang.String)[クラス MultiResourceItemWriterBuilder<T>]
- https://spring.pleiades.io/spring-batch/docs/current/api/org/springframework/batch/item/xml/builder/StaxEventItemReaderBuilder.html#name(java.lang.String)[クラス StaxEventItemReaderBuilder<T>]
- https://spring.pleiades.io/spring-batch/docs/current/api/org/springframework/batch/item/xml/builder/StaxEventItemWriterBuilder.html#name(java.lang.String)[クラス StaxEventItemWriterBuilder<T>]
...
====
FieldSetMapperを独自に実装する場合については、<<Ch05_FileAccess_HowToExtend>>を参照。
====
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050302" class="JavaConfigTab" id="tabgroup050302_1" checked><label for="tabgroup050302_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050302" class="XMLConfigTab" id="tabgroup050302_2"><label for="tabgroup050302_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050303" class="JavaConfigTab" id="tabgroup050303_1" checked><label for="tabgroup050303_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050303" class="XMLConfigTab" id="tabgroup050303_2"><label for="tabgroup050303_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
|icon:warning[] appendAllowedがtrueの場合は、この設定は無効化されるため、値を指定しないことを推奨する。 +
...
icon:warning[] 他の設定との組み合わせによって意図しない動作をする場合があるため、trueは設定しないことを推奨する。詳細は<<Ch05_FileAccess_HowToUse_VariableLength_Output_AboutShouldDeleteIfEmpty,後述>>する。
|
|false
...
``FieldExtractor``の実装例は<<Ch05_FileAccess_HowToUse_FixedLength_Output,固定長レコードの出力>>にて、全角文字のフォーマットを例に説明しているためそちらを参照。
|icon:check[]
|なし
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050304" class="JavaConfigTab" id="tabgroup050304_1" checked><label for="tabgroup050304_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050304" class="XMLConfigTab" id="tabgroup050304_2"><label for="tabgroup050304_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050305" class="JavaConfigTab" id="tabgroup050305_1" checked><label for="tabgroup050305_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050305" class="XMLConfigTab" id="tabgroup050305_2"><label for="tabgroup050305_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
https://github.com/spring-projects/spring-batch/issues/1139[{SB}/BATCH-2463] を参照。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050306" class="JavaConfigTab" id="tabgroup050306_1" checked><label for="tabgroup050306_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050306" class="XMLConfigTab" id="tabgroup050306_2"><label for="tabgroup050306_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050307" class="JavaConfigTab" id="tabgroup050307_1" checked><label for="tabgroup050307_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050307" class="XMLConfigTab" id="tabgroup050307_2"><label for="tabgroup050307_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050308" class="JavaConfigTab" id="tabgroup050308_1" checked><label for="tabgroup050308_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050308" class="XMLConfigTab" id="tabgroup050308_2"><label for="tabgroup050308_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
改行なし固定長ファイルから、指定したエンコーディングのバイト数で1レコードを読み込む``BufferedReader``生成クラス
...
この点についてはlink:https://github.com/spring-projects/spring-batch/issues/1062[{SB}/BATCH-2540]で報告しているため、今後不要になる可能性がある。
====

NOTE: FieldSetMapperの実装については、<<Ch05_FileAccess_HowToExtend>>を参照。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050309" class="JavaConfigTab" id="tabgroup050309_1" checked><label for="tabgroup050309_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050309" class="XMLConfigTab" id="tabgroup050309_2"><label for="tabgroup050309_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
|shouldDeleteIfEmpty
|trueの場合、出力件数が0件であれば出力対象ファイルを削除する。 +
icon:warning[] 他の設定との組み合わせによって意図しない動作をする場合があるため、trueは設定しないことを推奨する。詳細は<<Ch05_FileAccess_HowToUse_VariableLength_Output_AboutShouldDeleteIfEmpty,可変長レコードの出力の注意書き>>を参照。
|
|false
...
====
``FormatterLineAggregator``がもつ``fieldExtractor``プロパティのデフォルト値は``org.springframework.batch.item.file.transform.PassThroughFieldExtractor``である。

``PassThroughFieldExtractor``は、もとのアイテムに対して処理を行わずに返すクラスであり、``FieldExtractor``にて何も処理を行わない場合に使用する。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050310" class="JavaConfigTab" id="tabgroup050310_1" checked><label for="tabgroup050310_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050310" class="XMLConfigTab" id="tabgroup050310_2"><label for="tabgroup050310_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050311" class="JavaConfigTab" id="tabgroup050311_1" checked><label for="tabgroup050311_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050311" class="XMLConfigTab" id="tabgroup050311_2"><label for="tabgroup050311_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050312" class="JavaConfigTab" id="tabgroup050312_1" checked><label for="tabgroup050312_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050312" class="XMLConfigTab" id="tabgroup050312_2"><label for="tabgroup050312_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050313" class="JavaConfigTab" id="tabgroup050313_1" checked><label for="tabgroup050313_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050313" class="XMLConfigTab" id="tabgroup050313_2"><label for="tabgroup050313_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050314" class="JavaConfigTab" id="tabgroup050314_1" checked><label for="tabgroup050314_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050314" class="XMLConfigTab" id="tabgroup050314_2"><label for="tabgroup050314_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
.リスナー設定について
====
下記の2つの場合は自動で``Listener``として登録されないため、ジョブ定義時に``listenerメソッド/<batch:listeners>``にも定義を追加する必要がある。 +
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050315" class="JavaConfigTab" id="tabgroup050315_1" checked><label for="tabgroup050315_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050315" class="XMLConfigTab" id="tabgroup050315_2"><label for="tabgroup050315_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
Job/Stepの``ExecutionContext``に関する詳細は、<<Ch02_SpringBatchArchitecture.adoc#Ch02_SpringBatchArch,{SB}のアーキテクチャ>>を参照。
====
...
ステップの分割に関する詳細は、<<Ch08_FlowControll.adoc#Ch08_FlowControll,フロー制御>>を参照。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050316" class="JavaConfigTab" id="tabgroup050316_1" checked><label for="tabgroup050316_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050316" class="XMLConfigTab" id="tabgroup050316_2"><label for="tabgroup050316_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
詳しい理由は、<<Ch05_FileAccess_HowToUse_HeaderFooter_Input_AccessHeaders>>を参照。
|
|なし
|===
...
``Tasklet``の実装クラス内でBean定義した``ItemReader``を使用する方法は<<Ch03_CreateTaskletJob.adoc#Ch03_CreateTaskletJob_HowToUse_Impl,タスクレット指向ジョブの作成>>を参照。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050317" class="JavaConfigTab" id="tabgroup050317_1" checked><label for="tabgroup050317_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050317" class="XMLConfigTab" id="tabgroup050317_2"><label for="tabgroup050317_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050318" class="JavaConfigTab" id="tabgroup050318_1" checked><label for="tabgroup050318_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050318" class="XMLConfigTab" id="tabgroup050318_2"><label for="tabgroup050318_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
** ``resources``プロパティに読み込み対象のファイルを指定する
*** 正規表現で複数ファイルを指定する
** ``delegate``プロパティにファイル読み込みに利用する``ItemReader``を指定する
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050319" class="JavaConfigTab" id="tabgroup050319_1" checked><label for="tabgroup050319_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050319" class="XMLConfigTab" id="tabgroup050319_2"><label for="tabgroup050319_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
|``resource``プロパティは、``MultiResourceItemReader``から自動的に設定されるため、Bean定義に設定は不要である。
|icon:check[]
|
|===
...
** ``resources``プロパティに出力対象のファイルを指定する
*** ``ResourceSuffixCreator``の実装クラスで付与するサフィックスまでを設定
** ``resourceSuffixCreator``プロパティにサフィックスを生成する``ResourceSuffixCreator``の実装クラスを指定する
** ``delegate``プロパティにファイル読み込みに利用する``ItemWriter``を指定する
** ``itemCountLimitPerResource``プロパティに1ファイルあたりの出力件数を指定する

// ============ Java/XML Config Tab start. ============
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050320" class="JavaConfigTab" id="tabgroup050320_1" checked><label for="tabgroup050320_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050320" class="XMLConfigTab" id="tabgroup050320_2"><label for="tabgroup050320_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
|(3)
|delegate
|実際にファイルを書き込み処理する``ItemWriter``を設定する。
|icon:check[]
|なし
...
|(5)
|実際にファイルを書き込み処理する``ItemWriter``
|``resource``プロパティは、``MultiResourceItemWriter``から自動的に設定されるため、Bean定義に設定は不要である。
|icon:check[]
|
...
====
``itemCountLimitPerResource``は1ファイルのレコードの出力件数を決定するものだが、 +
この値は``commit-interval``で指定された数のレコードが出力された後で評価される。 +
このため、``itemCountLimitPerResource``で指定された値以上のレコードが1ファイルに出力される場合がある。 +

* ``itemCountLimitPerResource \<= commit-interval``の場合 +

{four_space}1ファイルのレコードの出力件数は``commit-interval``の値になる。 +
{four_space}例：入力データ全部で10件あり、``itemCountLimitPerResource=4, commit-interval=5``の場合 +
{four_space}{four_space}1ファイル目のレコードの出力件数は5件になる +
{four_space}{four_space}2ファイル目のレコードの出力件数は5件になる +

* ``itemCountLimitPerResource > commit-interval``の場合 +

{four_space}1ファイルのレコードの出力件数は``itemCountLimitPerResource``の値を超えたら、 +
{four_space}次のチャンクが新しく作成されたファイルに書き込まれる。 +
{four_space}例：入力データ全部で21件あり、``itemCountLimitPerResource=6, commit-interval=4``の場合 +
{four_space}{four_space}1ファイル目のレコードの出力件数は8件になる +
{four_space}{four_space}2ファイル目のレコードの出力件数は8件になる +
{four_space}{four_space}3ファイル目のレコードの出力件数は5件になる +
{four_space}{four_space}補足：最初4件は1ファイル目に出力された後``itemCountLimitPerResource``の6を超えていない、 +
{four_space}{four_space}そのため次の4件も1ファイル目に出力され、全部で8件になる。 +
{four_space}{four_space}さらに次の4件は来た時、8件はすでに``itemCountLimitPerResource``の6を超えているため、 +
{four_space}{four_space}この4件は2ファイル目に出力される。 +
{four_space}{four_space}結果として、1ファイル目と2ファイル目は8件になり、3ファイル目は5件になる +

よって、``itemCountLimitPerResource``で指定されたレコード件数でファイルの出力を分けるためには、 +
``commit-interval``と``itemCountLimitPerResource``の値を同数とするか、 +
``commit-interval``を``itemCountLimitPerResource``の約数となるように設定する。
====
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050321" class="JavaConfigTab" id="tabgroup050321_1" checked><label for="tabgroup050321_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050321" class="XMLConfigTab" id="tabgroup050321_2"><label for="tabgroup050321_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source,java]
...
|(2)
|``delegate-ref``属性に実際にファイルを読み込むItemReaderのBeanを指定する。
...
[[Ch05_FileAccess_HowToExtend_Xml_Mapping]]
=== オブジェクト変換ライブラリ
...
[[Ch05_FileAccess_HowToExtend_Xml_Encoding]]
==== 入出力におけるエンコーディングの仕様

XMLの入出力には{SB}が提供する``org.springframework.batch.item.xml.StaxEventItemReader``
および``org.springframework.batch.item.xml.StaxEventItemWriter``を使用する。

これらのコンポーネントのエンコーディングのデフォルト値は以下の表のとおり異なっているため、利用時には注意する必要がある。
デフォルト値の違いによって意図しないエンコーディングで入出力が行われることを防ぐため、
デフォルト値をそのまま使用する意図である場合でも明示的にエンコーディングを設定することを推奨する。

[cols="5,15,40,40", options="header"]
.各コンポーネントのエンコーディングについての仕様
|===
|項番
|コンポーネント名
|エンコーディングの指定方法
...
[WARNING]
.{SB}のバージョンアップに伴うエンコーディングの仕様変更
====
Macchinetta Batch 2.2.1以前が利用する{SB} 4.2.x以前の``StaxEventItemReader``のエンコーディングは入力するXMLファイルのencoding属性に従う。
encoding属性が宣言されていない場合はUTF-8で読み込みが行われるため、Bean定義で指定することはできなかった。 +
{SB} 4.3.0以降では``StaxEventItemReader``にencodingプロパティが追加され、デフォルト値を含め上記の表の仕様に変更された。

Macchinetta Batch 2.3.0は{SB} {spring_batch_version}を利用しているため、Macchinetta Batch 2.2.1以前からのバージョンアップを行った場合は、
エンコーディングを明示的に指定しないと読み込みに利用されるエンコーディングが変化してしまう可能性があるため注意すること。
====
...
** ``resource``プロパティに読み込み対象ファイルを設定する
** ``fragmentRootElementName``プロパティにルート要素の名前を設定する
** ``unmarshaller``プロパティに``org.springframework.oxm.jaxb.Jaxb2Marshaller``を設定する

* ``Jaxb2Marshaller``には以下のプロパティを設定する
** ``classesToBeBound``プロパティに変換対象のクラスをリスト形式で設定する
** スキーマファイルを用いたバリデーションを行う場合は、以下に示す2つのプロパティを設定する
*** ``schema``プロパティにバリデーションにて使用するスキーマファイルを設定する
*** ``validationEventHandler``プロパティにバリデーションにて発生したイベントを処理する``ValidationEventHandler``の実装クラスを設定する
...
.依存ライブラリの追加
====
JavaクラスとXML間のマッピングに``@XmlRootElement``などを使用するため、pom.xmlに下記の依存関係を追加する。
[source, xml]
----
<dependency>
...
Spring Frameworkが提供するライブラリであるSpring Object/XML Marshallingを使用する場合は、pom.xmlに下記の依存関係を追加する。
...
JAXBを利用する場合、jaxb-core及びjaxb-implが必要となる。
アプリケーションの依存ライブラリやバッチから提供されるライブラリにjaxb-core及びjaxb-implがない場合は、pom.xmlに下記の依存関係を追加する。
[source, xml]
----
<dependency>
...
|(1)
|<<Ch02_MacchinettaBatchStack#Ch02_MacchinettaBatchStack_Stack_OSS,「表 1. OSSバージョン一覧」>>のjaxb-coreのバージョンを指定する。

|(2)
|jaxb-coreはアプリケーションの実行時にのみ利用されるため、スコープはruntimeとする。

|(3)
|<<Ch02_MacchinettaBatchStack#Ch02_MacchinettaBatchStack_Stack_OSS,「表 1. OSSバージョン一覧」>>のjaxb-implのバージョンを指定する。
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050322" class="JavaConfigTab" id="tabgroup050322_1" checked><label for="tabgroup050322_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050322" class="XMLConfigTab" id="tabgroup050322_2"><label for="tabgroup050322_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
|encoding
|入力ファイルのエンコーディングを設定する。 +
<<Ch05_FileAccess_HowToExtend_Xml_Encoding>>で述べたように、ItemReader/Writer間のデフォルト値の違いによって意図しないエンコーディングで
入力が行われることを防ぐため、デフォルト値をそのまま使用する意図である場合でも明示的にエンコーディングを設定することを推奨する。
|
...
この例では、スキーマファイルの場所は``@Value``にファイルパス（絶対パス）で指定し、インジェクションする方式とする。 +
...
** ``resource``プロパティに出力対象ファイルを設定する
** ``marshaller``プロパティに``org.springframework.oxm.jaxb.Jaxb2Marshaller``を設定する
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050323" class="JavaConfigTab" id="tabgroup050323_1" checked><label for="tabgroup050323_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050323" class="XMLConfigTab" id="tabgroup050323_2"><label for="tabgroup050323_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050324" class="JavaConfigTab" id="tabgroup050324_1" checked><label for="tabgroup050324_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050324" class="XMLConfigTab" id="tabgroup050324_2"><label for="tabgroup050324_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
詳細は、<<Ch05_FileAccess_HowToExtend_Xml_Output>>を参照。
====
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050325" class="JavaConfigTab" id="tabgroup050325_1" checked><label for="tabgroup050325_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050325" class="XMLConfigTab" id="tabgroup050325_2"><label for="tabgroup050325_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
``PatternMatchingCompositeLineMapper``は、パターンマッチによってレコードに対して使用する``LineTokenizer``および``FieldSetMapper``を選択することができる。

たとえば、以下のような形で使用する``LineTokenizer``を選択することが可能である。

* ``USER*``にマッチする(レコードの先頭が``USER``である)場合は``userTokenizer``を使用する
* ``LINEA*``にマッチする(レコードの先頭が``LINEA``である)場合は``lineATokenizer``を使用する

[IMPORTANT]
.マルチフォーマットファイルを読み込む際のレコードにかかるフォーマットの制約
====
マルチフォーマットファイルを読み込むためには、レコード区分がAntPathMatcherによるパターンマッチで判別可能なフォーマットでなければならない。
詳細は``https://docs.spring.io/spring-batch/docs/{spring_batch_version}/api/org/springframework/batch/support/PatternMatcher.html[PatternMatcher]``のAPIドキュメントを参照。
====
...
** ``tokenizers``プロパティに各レコード区分に対応する``LineTokenizer``を設定する
** ``fieldSetMappers``プロパティに各レコード区分に対応する``FieldSetMapper``を設定する
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050326" class="JavaConfigTab" id="tabgroup050326_1" checked><label for="tabgroup050326_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050326" class="XMLConfigTab" id="tabgroup050326_2"><label for="tabgroup050326_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
...
``key``にレコードを判別するパターンを設定し、``value-ref``に使用する``LineTokenizer``を設定する。
|icon:check[]
|なし
...
``key``にレコードを判別するパターンを設定し、``value-ref``に使用する``FieldSetMapper``を設定する。
|icon:check[]
|なし
|===
...
** ``ItemWriter``の``lineAggregator``プロパティに``PassThroughLineAggregator``を設定する
...
++++
<div class="tabbox">
  <input type="radio" name="tabgroup050327" class="JavaConfigTab" id="tabgroup050327_1" checked><label for="tabgroup050327_1" class="tab">JavaConfig</label>
  <input type="radio" name="tabgroup050327" class="XMLConfigTab" id="tabgroup050327_2"><label for="tabgroup050327_2" class="tab">XMLConfig</label>
  <div class="tabcontent" name="JavaConfigContent">
++++
// Java Config start.
[source, java]
