...
非同期バッチデーモンは、org.terasoluna.batch.async.db.AsyncBatchDaemonの第一引数にBean定義ファイルのパス(``AsyncBatchDaemonConfig.java/async-batch-daemon.xml``)を指定して起動する。
...
$ java -cp dependency/* org.terasoluna.batch.async.db.AsyncBatchDaemon com.example.batch.tutorial.config.AsyncBatchDaemonConfig
----

この場合、``com.example.batch.tutorial.config.AsyncBatchDaemonConfig.java``を読み込み各種Beanを生成する。
...
XMLConfigでは、第一引数は省略可能である。その場合、``META-INF/spring/async-batch-daemon.xml``を読み込み各種Beanを生成する。
...
[NOTE]
.第一引数を省略した場合のBean定義ファイルパス
====
第一引数を省略した場合、Bean定義ファイルパスはXMLConfigのBean定義ファイルである``META-INF/spring/async-batch-daemon.xml``をデフォルトのパスとして起動する。

JavaConfigの場合、Bean定義ファイルはjavaパッケージ階層の中(``src/main/java``)に配置されるため、プロジェクトによってパスは異なり、デフォルト値としてあらかじめ決定することができない。 +
よって、JavaConfigにおいては第一引数のBean定義ファイルは省略せず必ず指定する必要がある。
====
...
<bean id="batchJobRequestRepository"
      ...
      p:mapperInterface="jp.co.ntt.fw.macchinetta.batch.functionaltest.ch04.asyncjobwithdb.repository.CustomizedBatchJobRequestRepository"
...
// Java Config start.
[source,java]
.jp.co.ntt.fw.macchinetta.batch.functionaltest.config.ch04.asyncjobwithdb.AsyncBatchDaemonClockConfig.java
----
@Bean
public JobRequestPollTask jobRequestPollTask(@Qualifier("adminTransactionManager") DataSourceTransactionManager adminTransactionManager,
                                             JobOperator jobOperator,
