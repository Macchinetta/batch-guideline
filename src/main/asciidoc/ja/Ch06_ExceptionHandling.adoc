...
|(3)
|``<batch:skippable-exception-classes>``を定義し、要素内に対象となる例外を設定する。
...
その機能は、以下のように``<chunk>``要素配下に``<skippable-exception-classes>``要素を設定し、スキップ対象の例外を指定する形で実装する。
...
* ``<skippable-exception-classes>``要素を利用して例外をスキップした場合、
