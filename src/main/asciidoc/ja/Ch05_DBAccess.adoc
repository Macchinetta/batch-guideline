...
|(1)
|``<mybatis:scan>``の``base-package``属性にMapper XMLをスキャンするパッケージを設定する。
指定するのはこのジョブが必要とするMapper XMLが直下に格納されているパッケージである。 +
``factory-ref``属性には``jobSqlSessionFactory``を設定する。
|===
...
|(3)
|``queryId``属性に、(7)で定義しているSQLのIDを(6)の``namespace`` + ``<メソッド名>``で指定する。

|(4)
|``sqlSessionFactory-ref``属性に、アクセスするデータベースの``SqlSessionFactory``を指定する。

|(5)
|(2)で定義した``MyBatisCursorItemReader``を``reader``属性に指定する。
...
|(3)
|``queryId``属性に、(6)で定義しているSQLのIDを(5)の``namespace`` + ``<メソッド名>``で指定する。
...
``ItemReader``にリソースの``open/close``メソッドを追加した``ItemStreamReader``インタフェースで実装をインジェクションする。
...
|(3)
|``<property>``要素の``name``属性に``parameterValues``を設定する。
...
タスクレットモデルで``apper``インタフェースを利用してデータベースを参照する実装例を以下に示す。 +
...
|(3)
|``statementId``属性に、(7)で定義しているSQLのIDを(6)の``namespace`` + ``<メソッド名>``で指定する。

|(4)
|``sqlSessionTemplate-ref``属性に、アクセスするデータベースの``SessionTemplate``を指定する。 +
指定する``SessionTemplate``は、``executorType``が``BATCH``に設定されていることが必須である。

|(5)
|(2)で定義した``MyBatisBatchItemWriter``を``writer``属性に指定する。
...
|(3)
|``statementId``属性に、(6)で定義しているSQLのIDを(5)の``namespace`` + ``<メソッド名>``で指定する。
...
|(11)
|``<list>``要素内に(8),(9)を設定する。指定した順番にItemWriterが実行される。
