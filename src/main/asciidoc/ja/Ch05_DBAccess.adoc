...
. 共通設定
** ``LaunchContextConfig.java/launch-context.xml``に読み込むMapper XMLを記述する。
. 個別設定
** ``jobs/``配下のBean定義に、個々のジョブごとに必要なMapper XMLを記述する。

共通設定、個別設定共に``@MapperScan/<mybatis:scan>``のスキャン範囲を不必要に広くしてしまうと、実行するジョブ以外のジョブが使用するMapper XMLも読み込んでしまう。これにより、以下に示す弊害が生じる。

* ジョブの起動までに時間がかかる
* メモリリソースの消費が大きくなる

これを回避するため、スキャン範囲は最小限になるようにどのジョブでも使われるMapper XMLは共通設定で読み込み、個々のジョブ定義でそのジョブが必要とするMapper XMLは個別設定で読み込むなど使い分ける必要がある。
