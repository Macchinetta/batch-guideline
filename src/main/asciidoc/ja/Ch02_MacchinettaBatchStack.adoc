...
https://docs.spring.io/spring-boot/{spring_boot_version}/index.html[{spring_boot_version}]である。
====
...
|org.springframework
|https://docs.spring.io/spring-framework/reference/{spring_fw_version}/web/webmvc.html#mvc[spring-webmvc]
|{spring_fw_version}
...
.2+|ログ出力
.2+|実装にLogbackを利用し、SLF4Jはロギングの抽象化レイヤーを提供する。
|ch.qos.logback
...
|https://docs.hibernate.org/validator/8.0/reference/en-US/html_single/[hibernate-validator]
|8.0.3.Final
...
|https://github.com/FasterXML/jackson-databind/blob/2.19/README.md[jackson-databind]
|2.19.4
...
[[Ch02_MacchinettaBatchStack_TerasolunaBOM]]
=== {terabom_fullname}
{terabom_fullname} (https://github.com/terasolunaorg/terasoluna-dependencies/tree/2.0.1.RELEASE[terasoluna-dependencies])は{batch5_fullname}で使用するOSSライブラリのバージョンを一元管理するためのBOM(Project Object Model)プロジェクトである。
terasoluna-dependenciesでは以下を定義している。

** {batch5_fullname}が参照する https://docs.spring.io/spring-boot/3.5/reference/using/build-systems.html#using.build-systems.dependency-management[spring-boot-dependencies]のバージョン

** spring-boot-dependenciesが管理しているライブラリ以外で{batch5_fullname}が参照するライブラリのバージョン

terasoluna-dependenciesで管理されているライブラリのバージョンをそのまま利用する場合は、アプリケーション側のpom.xmlにおいて、ライブラリのバージョンを定義する必要はない。

terasoluna-dependenciesで定義しているライブラリとそのバージョンについては、 https://github.com/terasolunaorg/terasoluna-dependencies/wiki#managed-dependency-coordinates[Managed Dependency Coordinates]で公開している。
