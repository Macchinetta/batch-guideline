...
ifeval::["{docname}" == "single_index"]
また、アプリケーションの背景、処理概要、業務仕様は、<<_Ch09_Impl_single_index.adoc#Ch09_Impl,バッチジョブの実装>>の各チュートリアルジョブを参照。
endif::[]
...
Database Browserの表示手順は<<Ch09_EnvironmentConstruction.adoc#Ch09_EnvironmentConstruction_OperationCheck_RefDB,DBeaverを使用してデータベースを参照する>>を参照。
...
.SQL Editor
image::ch09/AsyncExecution/Ch09_AsyncExecutionJob_SQLEditor.png[Open SQL Editor]
